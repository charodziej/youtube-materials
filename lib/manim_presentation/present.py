# Sourced from https://github.com/galatolofederico/manim-presentation/blob/main/manim_presentation/present.py
# Modified to improve performance and changing slides in different presentations
...
class Config:
    ...
    def init(cls):
        ...
        if os.path.exists(
            os.path.join(os.getcwd(), "./manim-presentation.json")
        ):
            json_config = json.load(
                open(
                    os.path.join(os.getcwd(), "./manim-presentation.json"), "r"
                )
            )
...
class Presentation:
    ...
    # Also returns a boolean which determines if the video should be refreshed
    def update_state(self, state):
        if state == State.PAUSED:
            if len(self.lastframe) == 0:
                _, self.lastframe = self.current_cap.read()
            return self.lastframe, state, True
        still_playing, frame = self.current_cap.read()
        if still_playing:
            self.lastframe = frame
        elif state in [state.WAIT, state.PAUSED]:
            return self.lastframe, state, False
        elif (
            self.current_slide["type"] == "last"
            and self.current_slide["terminated"]
        ):
            return self.lastframe, State.END, True

        if not still_playing:
            if (
                self.current_slide["end_animation"]
                ...
                if self.current_slide["type"] == "slide":
                    ...
                    self.current_animation = self.current_slide[
                        "start_animation"
                    ]
                    state = State.PLAYING
                    self.rewind_slide()
                ...
                self.current_slide["type"] == "last"
                and self.current_slide["end_animation"]
...
class Display:
    ...
    def run(self):
        while True:
            (
                ...
                self.state,
                should_render,
            ) = self.current_presentation.update_state(self.state)
    ...
    def show_info(self):
        ...
        cv2.putText(
            info, f"State: {self.state}", (grid_x[1], grid_y[0]), *font_args
        )
    ...
    def handle_key(self):
        ...
        if key == Config.QUIT_KEY:
            ...
            key == Config.CONTINUE_KEY or key == Config.PLAYPAUSE_KEY
        ):
            ...
            if self.current_presentation.current_slide_i == 0:
                ...
                self.current_presentation.current_slide["terminated"] = False
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Scenes to present",
    )
    parser.add_argument(
        "--folder",
        type=str,
        default="./presentation",
        help="Presentation files folder",
    )
    parser.add_argument(
        "--start-paused", action="store_true", help="Start paused"
    )
    ...
    for scene in args.scenes:
        ...
        presentations.append(
            Presentation(config, last_frame_next=args.last_frame_next)
        )
